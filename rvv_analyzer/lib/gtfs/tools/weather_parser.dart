...
class WeatherParser {
  static Map<DateTime, WeatherRecord> parseWeather(String csvContent) {
    // Using Csv().decode as done in RecordedDataParser to match package version
    final rows = Csv(
      lineDelimiter: '\n',
      dynamicTyping: false,
    ).decode(csvContent);
    ...
    // The header is: time,temp,dwpt,rhum,prcp,snow,wdir,wspd,wpgt,pres,tsun,coco
    ...
    for (var i = 1; i < rows.length; i++) {
      ...
      try {
        ...
        // We replace space with 'T' and add 'Z' to force parsing as UTC.
